			...
			while (mQueue.length > 0) {
				...
				for each (touch in mCurrentTouches) {
					...
					if (touch.target && touch.target.stage == null)
						touch.setTarget(mStage.hitTest(new Point(touch.globalX, touch.globalY), true));
				}
				...
				for each (var touchData:Object in sHoveringTouchData)
					if (touchData.touch.target != touchData.target)
						touchData.target.dispatchEvent(new TouchEvent(TouchEvent.TOUCH, mCurrentTouches, mShiftDown, mCtrlDown));
				...
				for each (touchID in sProcessedTouchIDs) {
					...
					if (touch.target)
						touch.target.dispatchEvent(new TouchEvent(TouchEvent.TOUCH, mCurrentTouches, mShiftDown, mCtrlDown));
				}
