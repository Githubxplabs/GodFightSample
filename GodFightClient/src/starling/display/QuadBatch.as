...
package starling.display {
	...
	public class QuadBatch extends DisplayObject {
		...
		public static function compile(container : DisplayObjectContainer, quadBatches : Vector.<QuadBatch>) : void {
			compileObject(container, quadBatches, -1, new Matrix());
		}

		private static function compileObject(object : DisplayObject, quadBatches : Vector.<QuadBatch>, quadBatchID : int, transformationMatrix : Matrix, alpha : Number = 1.0, blendMode : String = null) : int {
			var i : int;
					...
					for each (var mipmap:Boolean in [true, false]) {
						for each (var smoothing:String in smoothingTypes) {
							for each (var format:String in formats) {
								...
								target.registerProgram(getImageProgramName(tinted, mipmap, repeat, format, smoothing), vertexProgramAssembler.agalcode, fragmentProgramAssembler.agalcode);
							}
						}
					}
				}
