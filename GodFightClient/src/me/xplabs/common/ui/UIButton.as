...
package me.xplabs.common.ui {
	 ...
	 */
	public class UIButton extends DisplayObjectContainer {
		...
		public function UIButton(upState : Texture, text : String = "", downState : Texture = null, overState:Texture = null) {
		...
		private function onTouch(event : TouchEvent) : void {
			//Mouse.cursor = (mUseHandCursor && mEnabled && event.interactsWith(this)) ? MouseCursor.BUTTON : MouseType.normalHand;
			
			if (mUseHandCursor && mEnabled && event.interactsWith(this))
			{
				Mouse.cursor = MouseCursor.BUTTON;
				if(mOverState) mBackground.texture = mOverState;
			}else
			{
				if (mOverState) mBackground.texture = mUpState;
				Mouse.cursor = MouseType.normalHand
			}
